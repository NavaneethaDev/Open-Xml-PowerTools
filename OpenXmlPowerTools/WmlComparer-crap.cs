...
namespace OpenXmlPowerTools
{
    ...
    public static class WmlComparer
    {
        ...
        {
            var cu1 = ComparisonUnit.GetComparisonUnitList(sra1, settings);
            var cu2 = ComparisonUnit.GetComparisonUnitList(sra2, settings);
    ...
    internal class ComparisonUnit : IEquatable<ComparisonUnit>
    {
        ...
        public List<ComparisonUnit> Group;

        private static bool s_DumpLog = false;
        ...
        public ComparisonUnit(IEnumerable<ComparisonUnit> comparisonUnitList)
        {
            Group = comparisonUnitList.ToList();
            Contents = null;
        }
        ...
        public static ComparisonUnit[] GetComparisonUnitList(ContentAtomListAnnotation contentAtomListAnnotation, WmlComparerSettings settings)
        {
            ...
            var withStoryKey = groupedByWords
                .Select(gbw =>
                {
                    var ancestors = gbw
                        .First()
                        ...
                        .Select(a =>
                        {
                            ...
                            if (unid == null)
                                throw new OpenXmlPowerToolsException("Internal error - why is unid null?");
                            return a.Name.LocalName + "[" + unid + "]/";
                        })
                        .StringConcatenate();
                    var newComparisonUnit = new ComparisonUnit();
                    newComparisonUnit.Contents = 
                    return new {
                        Key = ancestors,
                        ComparisonUnit = new ComparisonUnit(),
                    };
                })
                .ToList();
            ...
            {
                var sb = new StringBuilder();
                foreach (var z in groupedByStoryKey)
                {
                    sb.Append("Key: >" + z.Key.PadRight(48, '-') + "< " + Environment.NewLine);
                    sb.Append("ComparisonUnit: " + z.ComparisonUnit.ToString() + Environment.NewLine);
                }
                var sbs = sb.ToString();
                Console.WriteLine(sbs);
            }

            // I could use the approach here of creating hierarchy from flat, as detailed in MSDN blog, https://blogs.msdn.microsoft.com/ericwhite/2008/07/10/how-to-create-hierarchy-from-flat-data-using-linq/
            // However, perf is important in this routine, so coding imperative style.
            //int level = 0;
            ...
            //string lastKey = "";
            //List<ComparisonUnit> cul = new List<ComparisonUnit>();
            //foreach (var gbs in groupedByStory)
            //{
            //    if (gbs.Key == lastKey)
            //    {
            //        var first = gbs.First();
    ...
    internal class StoryGroup {
        public string Key;
        public IEnumerable<ComparisonUnit> ComparisonUnitList;
    };
